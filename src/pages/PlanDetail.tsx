...
export default function PlanDetail() {
  ...
  const discountAmount = appliedPromo
    ? Math.round((plan.price * appliedPromo.discount) / 100)
    : 0
  ...
  const toggleAddOn = (upsell: Upsell) => {
    setSelectedAddOns((prev) =>
      prev.find((a) => a.id === upsell.id)
        ? prev.filter((a) => a.id !== upsell.id)
        : [...prev, upsell]
    )
  }

  const handleOrder = () => {
    // Store state in sessionStorage for order form
    ...
    <main style={{ paddingTop: 100 }}>
      <section style={{ padding: '24px 16px 80px', maxWidth: 700, margin: '0 auto' }}>
        ...
        >
          ...
          <h1 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 32, fontWeight: 700, marginBottom: 4 }}>
            {plan.name}
          </h1>
          <p style={{ color: 'var(--muted)', fontSize: 15, marginBottom: 20 }}>{plan.tagline}</p>
            ...
            {appliedPromo ? (
              <div>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
                  <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 40, fontWeight: 700, color: 'var(--teal)' }}>
                    {formatPrice(discountedPrice)}
                  </span>
                  <span style={{ color: 'var(--muted)', fontSize: 15 }}>/ one-time</span>
                </div>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 40, fontWeight: 700 }}>
                  {formatPrice(plan.price)}
                </span>
                <span style={{ color: 'var(--muted)', fontSize: 15 }}>/ one-time</span>
              </div>
        ...
        <button
          ...
          style={{ width: '100%', justifyContent: 'center', fontSize: 16, padding: '15px', marginBottom: 8 }}
        >
          Place Order →
        </button>
        <p style={{ textAlign: 'center', color: 'var(--muted)', fontSize: 12, marginBottom: 32 }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          {plan.domainNote && (
            ...
            >
              <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', color: '#f5c842', fontFamily: 'Space Grotesk, sans-serif', marginBottom: 8 }}>
                ⚠ IMPORTANT DOMAIN INFORMATION
              </p>
              <p style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', marginBottom: 8, fontFamily: 'Space Grotesk, sans-serif' }}>
                Custom domains require annual renewal
              </p>
              {plan.domainNote.split('. ').map((line, i) => (
                <p key={i} style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 4, lineHeight: 1.5 }}>
                  • {line}
                </p>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {plan.whatToExpect.map((item) => (
              <div key={item} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                <Star size={14} color="var(--purple)" style={{ marginTop: 2, flexShrink: 0 }} />
        ...
        >
          <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', color: 'var(--muted)', fontFamily: 'Space Grotesk, sans-serif', marginBottom: 6 }}>
            READY TO START?
          </p>
          <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 18, fontWeight: 700, marginBottom: 16 }}>
        ...
        {/* Suggested services (upsells) */}
        {upsells.length > 0 && (
          <div>
            <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', color: 'var(--muted)', fontFamily: 'Space Grotesk, sans-serif', marginBottom: 16 }}>
              SUGGESTED SERVICES
            </p>
            <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 20, fontWeight: 700, marginBottom: 16 }}>
              Boost your launch
              ...
              {upsells.map((upsell) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      padding: '18px 18px',
                      border: selected ? '1px solid var(--teal)' : undefined,
                      background: selected ? 'rgba(0,212,184,0.05)' : undefined,
                    }}
                  >
                    ...
                    <h3 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 16, fontWeight: 700, marginBottom: 4 }}>
                      {upsell.name}
                    </h3>
                    ...
                    <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 12 }}>
                      Pay once — added to your order total.
                    </p>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ color: 'var(--teal)', fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 18 }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          border: `1px solid ${selected ? 'var(--teal)' : 'var(--border-strong)'}`,
                          color: selected ? '#0a0d14' : 'var(--text)',
                          borderRadius: 8,
                          padding: '6px 14px',
                          fontSize: 13,
                          fontWeight: 600,
            ...
            {selectedAddOns.length > 0 && (
              ...
              >
                <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 8 }}>TOTAL</p>
                <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 28, fontWeight: 700, color: 'var(--teal)' }}>
                  {formatPrice(total)}
                  {appliedPromo && (
                    <span style={{ color: 'var(--muted)', fontSize: 13, fontWeight: 400, marginLeft: 8, textDecoration: 'line-through' }}>
                      {formatPrice(plan.price + addOnsTotal)}
                    </span>
                  )}
                </p>
                <button
